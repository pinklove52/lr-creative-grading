...
test("stage gating exposes exactly the M1/M2/M3 method sets from the plan", async () => {
  ...
  assert.ok(activeStage, "Config.stage must be one of M1/M2/M3");
  assert.ok(["M1", "M2", "M3"].includes(activeStage));
});
...
test("QueueTransport.lua enforces the full preflight chain and quarantine codes", async () => {
  ...
  assert.match(source, /Bridge\.invoke/);
  // 所有异步入口都有 pcall 兜底
  assert.ok((source.match(/pcall/g) ?? []).length >= 8);
  // 每个请求只在 dispatch 外保留一个 yield-safe 边界，不能嵌套包裹整个处理任务。
  assert.match(source, /LrTasks\.pcall\(dispatch, request\)/);
  assert.match(source, /return true, Bridge\.invoke\(request\.method, request\.params or \{\}\)/);
  assert.doesNotMatch(source, /pcall\(processClaimed, request, claimPath\)/);
});
