...
- [ ] 切换 stage 后 `capabilities` 的 `enabled_methods` 增加了 `get_target_photo / get_settings / get_proxy`
- [ ] 选中测试照片 → `get_target_photo` 返回的文件名/digest 与实际一致
- [ ] `get_settings` 读回的参数值与 Lightroom 修改照片面板一致（曝光、白平衡等 5 个基础参数）
- [ ] `get_proxy` 在 `%TEMP%\LrCreativeGradingBridge\proxies\` 生成 JPEG 代理，内容与当前编辑一致
- [ ] 代理渲染期间快速切换到另一张照片 → 返回 `TARGET_MISMATCH` 或 `PROXY_STALE`
- [ ] 修改照片（手动改曝光）→ `get_settings` 的 baseline digest 变化
- [ ] 连续 50 次只读调用无卡顿、无任务静默终止（`bridge.log` 无 error）
- [ ] 图库/修改照片模块来回切换，读操作结果不受影响
