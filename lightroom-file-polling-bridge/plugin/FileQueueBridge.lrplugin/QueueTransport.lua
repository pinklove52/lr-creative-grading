...
local transportMethods = {
    ...
    capabilities = function()
        local bridgeOk, bridgeResult = Bridge.handle("capabilities", {})
        local bridge = bridgeOk and bridgeResult or { error = bridgeResult }
        return true, {
            protocol_version = Config.protocol_version,
            plugin_version = pluginVersion(),
            lr_version = lrVersion(),
...
local function dispatch(request)
    ...
    return Bridge.handle(request.method, request.params or {})
end
...
local function pollOnce()
    ...
    LrTasks.startAsyncTask(function()
        local runOk, runResult = pcall(processClaimed, request, claimPath)
        if not runOk then
            Queue.lastError = "processing task crashed: " .. tostring(runResult)
            Queue.log("error", "processing_task_crashed", { message = tostring(runResult) })
            pcall(function() LrFileUtils.delete(claimPath) end)
            Queue.failedRequests = Queue.failedRequests + 1
            Queue.busy = false
        end
