...
local function sha256(value)
    local ok, digest = pcall(function() return LrDigest.SHA256.digest(value) end)
...
local function currentModule()
    local ok, moduleName = pcall(LrApplicationView.getCurrentModuleName)
...
local function ensureDevelopModule()
    ...
    local switched, reason = pcall(LrApplicationView.switchToModule, "develop")
...
local function controllerRange(entry)
    local ok, minimum, maximum = pcall(LrDevelopController.getRange, entry.lr)
...
local function controllerValue(entry)
    local ok, value = pcall(LrDevelopController.getValue, entry.lr)
...
local function capabilities()
    ...
    local hasPhoto, _, photo = pcall(activePhoto)
    if hasPhoto and photo then
        local ok, settings = pcall(developSettings, photo)
        if ok then photoSettings = settings end
    end
...
local function getProxy(params)
    ...
    local pending = { done = false, bytes = nil, last_error = nil, callbacks = 0 }
    Bridge.pendingProxy[requestId] = pending
    local ok, requestOrReason = pcall(function()
        return photo:requestJpegThumbnail(longEdge, longEdge, function(jpegBytes, reason)
            ...
            if type(jpegBytes) == "string" and #jpegBytes > 0 then
                ...
                pending.done = true
            elseif reason then
                -- Preview generation can invoke the callback more than once;
                -- retain the latest failure but continue waiting for success.
                pending.last_error = tostring(reason)
            end
        end)
    end)
    if not ok then
        Bridge.pendingProxy[requestId] = nil
        bridgeError("PROXY_REQUEST_FAILED", "Lightroom rejected the JPEG thumbnail request", tostring(requestOrReason))
    end
    pending.request_reference = requestOrReason
    local waited = 0
    while not pending.done and waited < timeoutSeconds do
        LrTasks.sleep(0.05)
        waited = waited + 0.05
    end
    Bridge.pendingProxy[requestId] = nil
    if not pending.done or not pending.bytes then
...
local function validatePreset(recipe)
    ...
    local ok, settings = pcall(function() return preset:getSetting() end)
    if not ok or type(settings) ~= "table" then
...
local function findSnapshotId(photo, name)
    local ok, snapshots = pcall(function() return photo:getDevelopSnapshots() end)
...
local function applyTransaction(request)
    ...
    local applyOk, applyReason = pcall(function()
        ...
        if #plan.ordered > 0 then
            local thresholdSet = pcall(LrDevelopController.setMultipleAdjustmentThreshold, 1.5)
            local numericOk, numericReason = pcall(function()
                for _, entry in ipairs(plan.ordered) do
                    ...
                    if not plan.preset and valuesEqual(beforeValue, desired) then
                        ...
                        local setOk, setReason = pcall(LrDevelopController.setValue, entry.lr, desired)
                        if not setOk then
            ...
            if thresholdSet then pcall(LrDevelopController.setMultipleAdjustmentThreshold, 0.5) end
...
function Bridge.handle(method, params)
    ...
    if not handler then
        return false, { code = "METHOD_NOT_FOUND", message = "Unknown bridge method: " .. tostring(method) }
    end
